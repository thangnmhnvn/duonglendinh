Object.assign(GameLogic, {
    ...
    startKhoiDong: function() {
        GameUI.playVideo(GameConfig.paths.khoiDongIntro, function() {
            ...
            GameUI.playBgMusic({'url': GameConfig.paths.audio.startRound}, () => {});
        });
    },
    ...
    preparePlayerTurn: function(playerIndex) {
        ...
        if (!questions) return GameUI.showNotification("Lỗi dữ liệu câu hỏi!");
    ...
    startTimer: function() {
        ...
        this.timer = setInterval(() => {
            ...
            if (this.timeLeft == 5) GameUI.playMusic({'url':GameConfig.paths.audio.c5giay}, () => {});
        }, 1000);
    },

    finishTurn: function() {
        clearInterval(this.timer);
        GameUI.playMusic({'url':GameConfig.paths.audio.finishRound}, () => {});

        GameUI.stopAllMedia();
        GameUI.showNotification("Kết thúc lượt của " + GameConfig.players[this.currentPlayerIndex].name);
        ...
        if (next < GameConfig.players.length) {
            ...
            GameUI.showNotification("Đã kết thúc Vòng Khởi Động! Chuyển sang Vòng 2.", function () {
                GameLogic.transitionToRound2();
            });
        }
